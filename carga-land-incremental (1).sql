INSERT INTO `roxpartner-estagiarios-infra.LAND_PROTHEUS.land_fornecedores` (
  codigo_fornecedor, nome_fornecedor, cnpj, ativo, endereco, numero, complemento, cep, bairro, email, data_cadastro,          
  data_atualizacao
)
WITH tabela as (
  SELECT
    ...
    WHERE data_atualizacao = (select MAX(data_atualizacao) from `roxpartner-estagiarios-infra.PROD_PROTHEUS.prod_fornecedores`)
)
SELECT * FROM tabela;